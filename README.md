...
项目早期曾把 Agent Reach 当作获取层。自 `v0.7.0` 起，这一位置已经由项目自有的 Provider Registry、Route Plan 和 Bundle Writer 替代；`v0.7.1` 又补齐了 X 内嵌视频、YouTube 无字幕转 ASR 和学习层接力：

| 维度 | Agent Reach | 本项目 v0.7.1 |
| --- | --- | --- |
...
内置结构化直连路线覆盖普通网页、YouTube、Bilibili、GitHub、X/Twitter、小红书和 Exa 搜索。X 帖子正文与帖子内嵌视频是两个独立能力：前者走文本读取，后者走字幕提取，并在字幕不可用时把媒体交给证据层 ASR。RSS、Reddit、Facebook、Instagram、LinkedIn、小宇宙、V2EX、雪球等渠道通过统一的授权导入契约进入同一个 Bundle v2。
...
```powershell
kw run --input <URL或本地材料> --target <目标> --operation <操作> `
  --mode audit --deliverable learning_article --learning-depth standard
```

如果总控台生成 `15_learning/learning_enrichment_request.json`，说明确定性程序已经完成获取和审计，正在等待 Agent 依据已验收 transcript 写入证据锚定的 `learning_enrichment.json`。完成后执行：

```powershell
kw learn --project-root <任务目录> --enrichment <任务目录>\15_learning\learning_enrichment.json
```
