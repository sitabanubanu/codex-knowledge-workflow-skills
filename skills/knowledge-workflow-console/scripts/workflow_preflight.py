...
def build_preflight(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if input_kind == "transcript_or_subtitle":
        ...
        route = "native_acquisition_bundle_then_source_gate"
...
def next_step_for(input_kind: str, mode: str) -> str:
    if input_kind == "url":
        return "Run the native provider doctor, create an acquisition bundle, then ingest through the source gate."
    if input_kind == "media":
