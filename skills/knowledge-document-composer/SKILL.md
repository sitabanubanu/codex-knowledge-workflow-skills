...
6. Prefer `kw audit --project-root <project>` so planning output is
...
11. Prefer `kw compose --project-root <project>`. It validates the
...
- Do not read provider or `00_acquisition` output directly. Composer only consumes source-gated packs and planning artifacts.
